...
import (
	...
	"github.com/fengde/gocommon/confx/stringxx"
)
...
func parseKeyAndOptions(tagName string, field reflect.StructField) (string, *fieldOptions, error) {
	...
	cacheLock.RUnlock()
	if ok {
		return stringxx.TakeOne(cache.key, field.Name), cache.options, cache.err
	}
	...
	return stringxx.TakeOne(key, field.Name), options, err
}
